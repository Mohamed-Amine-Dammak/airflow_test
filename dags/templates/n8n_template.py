...
def build_n8n_dag(config: dict):
    dag_id = config["dag_id"]
    start_date = datetime.fromisoformat(config["start_date"])
    tags = config.get("tags", ["n8n"])
    connection_id = config.get("connection_id", "n8n_local")
    ...
    def n8n_dynamic_workflow_dag():
        ...
        for job_cfg in jobs:
            ...
            for upstream_id in job_cfg.get("depends_on", []):
                task_map[upstream_id]["monitor"] >> task_map[current_job_id]["trigger"]

    return n8n_dynamic_workflow_dag()