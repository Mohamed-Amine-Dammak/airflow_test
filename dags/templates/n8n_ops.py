...
def n8n_api_headers(api_key: str) -> dict:
    return {
...
def pause(label: str, seconds: int = 10):
    print(f"{label} -> sleeping for {seconds} seconds...")
    time.sleep(seconds)
    print(f"{label} -> resumed.")


def extract_webhook_paths(workflow_obj: dict) -> list[dict]:
    ...
    for node in nodes:
        ...
        if isinstance(path, str) and path.strip():
            results.append({
                ...
                "webhook_path": normalize_path(path),
            })

    return results


def find_workflow_by_name(conn, workflow_name: str) -> dict:
    base_url = conn.host.rstrip("/")
    headers = n8n_api_headers(conn.password)
...
def get_workflow_details(conn, workflow_id: str) -> dict:
    base_url = conn.host.rstrip("/")
    headers = n8n_api_headers(conn.password)
...
def print_execution_details(
    ...
    for node_name, runs in run_data.items():
        ...
        for i, run in enumerate(runs, 1):
            ...
            start_time_val = run.get("startTime") or run.get("startedAt")
            end_time_val = run.get("executionTime") or run.get("stoppedAt")
            if start_time_val:
                print(f"  │   start: {start_time_val}")
            if end_time_val:
                print(f"  │   end/executionTime: {end_time_val}")
...
def make_resolve_workflow_task(connection_id: str):
    @task
    def resolve_workflow(
        ...
        workflow = find_workflow_by_name(conn, workflow_name)
        workflow_id = str(workflow.get("id"))

        if not workflow_id:
            raise AirflowException(f"Workflow '{workflow_name}' found but no id returned")

        details = get_workflow_details(conn, workflow_id)
        webhooks = extract_webhook_paths(details)
        ...
        pause(f"After resolve_workflow [{workflow_name}]", sleep_after)
...
def make_trigger_n8n_task(connection_id: str):
    @task
    def trigger_n8n_workflow(
        ...
        pause(f"After trigger_n8n_workflow [{workflow_name}]", sleep_after)
...
def make_monitor_n8n_task(connection_id: str):
    @task
    def monitor_n8n_workflow(
        ...
        pause(f"After monitor_n8n_workflow [{workflow_name}]", sleep_after)

        return final_status

    return monitor_n8n_workflow