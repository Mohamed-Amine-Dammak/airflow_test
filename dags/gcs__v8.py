...
_TASK_RESUME_SCOPE_BY_ENTRY = {'gcs_1_node_4': ['gcs_1_node_4',
                  'gcs_3_node_5',
                  'gcs_4_node_8',
                  'gcs_6_node_6',
                  'parallel_fork_2_node_7',
                  'parallel_join_5_node_9'],
 'parallel_fork_2_node_7': ['gcs_3_node_5',
                            'gcs_4_node_8',
                            'gcs_6_node_6',
                            'parallel_fork_2_node_7',
                            'parallel_join_5_node_9'],
 'gcs_3_node_5': ['gcs_3_node_5', 'gcs_6_node_6', 'parallel_join_5_node_9'],
 'gcs_4_node_8': ['gcs_4_node_8', 'gcs_6_node_6', 'parallel_join_5_node_9'],
 'parallel_join_5_node_9': ['gcs_6_node_6', 'parallel_join_5_node_9'],
 'gcs_6_node_6': ['gcs_6_node_6']}
...
def gcs_v8():
    ...
    def run_node_4():
        ...
        local_file_path = '/opt/airflow/local_uploads/gcs/node_4/elk.docx'
        local_directory_path = ''
        local_file_paths = ['/opt/airflow/local_uploads/gcs/node_4/elk.docx',
     '/opt/airflow/local_uploads/gcs/node_4/n8n_vs_langchain_langGraph.docx']
        drive_file_link = ''
        ...
        delete_mode = 'single'
        object_name = 'dataops.png'
        object_names = []
        delete_multiple_source_type = 'local'
        delete_object_prefix = ''
        delete_local_directory_path = ''
        delete_local_file_paths = []
        delete_drive_input_mode = 'directory'
        delete_drive_directory_link = ''
        delete_drive_file_links = []
        ...
        if action == "delete":
            if delete_mode == "single":
                ...
                targets = []
                source_type = str(delete_multiple_source_type or "local").strip().lower()
                prefix = str(delete_object_prefix or "").strip().strip("/")

                if source_type == "local":
                    local_files = [str(item).strip() for item in (delete_local_file_paths or []) if str(item).strip()]
                    if not local_files:
                        local_dir = str(delete_local_directory_path or "").strip()
                    ...
                    if not local_files and isinstance(object_names, list):
                        # Backward compatibility: existing pipelines may still provide direct object names.
                        targets = [str(item).strip() for item in object_names if str(item).strip()]
                    else:
                        for file_path in local_files:
                            name = os.path.basename(str(file_path))
                            if not name:
                                continue
                            targets.append(f"{prefix}/{name}" if prefix else name)
                elif source_type == "drive":
                    drive_service = _build_drive_service(service_account_file)
                    drive_file_ids = []
                    if str(delete_drive_input_mode or "directory").strip().lower() == "directory":
                        folder_id = _extract_drive_id(delete_drive_directory_link, "folder")
                        if not folder_id:
                            raise AirflowException("Invalid gcs.delete_drive_directory_link for multiple Drive delete.")
                        page_token = None
                        while True:
                            resp = drive_service.files().list(
                                q=f"'{folder_id}' in parents and trashed = false",
                            ...
                            for item in resp.get("files", []):
                                ...
                                if item.get("name"):
                                    ...
                                    drive_file_ids.append(str(item.get("id")))
                            page_token = resp.get("nextPageToken")
                            if not page_token:
                                break
                    else:
                        drive_file_ids = [
                            _extract_drive_id(link, "file")
                            for link in (delete_drive_file_links or [])
                            if str(link or "").strip()
                        ]
                        drive_file_ids = [item for item in drive_file_ids if item]

                    for file_id in drive_file_ids:
                        metadata = drive_service.files().get(fileId=file_id, fields="id,name").execute()
                        name = str(metadata.get("name") or "").strip()
                        if not name:
                            continue
                        targets.append(f"{prefix}/{name}" if prefix else name)
                else:
                    raise AirflowException(f"Unsupported gcs.delete_multiple_source_type: {source_type}")
    ...
    task_node_7 = EmptyOperator(
        task_id='parallel_fork_2_node_7',
    )

    @task(task_id='gcs_3_node_5')
    def run_node_5():
        action = 'move'
        bucket_name = 'os-dpf-ariflow-prj-dev'
        service_account_file = '/opt/airflow/credgcp.json'
        ...
        source_type = 'local'
        multiple_source_type = 'local'
        local_file_path = '/opt/airflow/local_uploads/gcs/node_4/dataops.png'
        local_directory_path = ''
        local_file_paths = []
        drive_file_link = ''
        drive_input_mode = 'directory'
        drive_directory_link = ''
        drive_file_links = []
        source_object = 'CUSTOMER_CUS_TEST.csv/CUSTOMER_CUS_TEST.csv'
        destination_bucket_name = 'os-dpf-ariflow-prj-shr-dev'
        destination_object = 'CUSTOMER_CUS_TEST.csv'
        delete_mode = 'single'
        object_name = 'dataops.png'
    ...
    def run_node_8():
        ...
        drive_directory_link = ''
        drive_file_links = []
        source_object = 'CUSTOMER_CUS_TEST.csv/elk.docx'
        destination_bucket_name = 'os-dpf-ariflow-prj-shr-dev'
        destination_object = 'elk.docx'
        delete_mode = 'single'
        object_name = 'dataops.png'
        object_names = []
        delete_multiple_source_type = 'local'
    ...
    task_node_9 = EmptyOperator(
        task_id='parallel_join_5_node_9',
        trigger_rule='all_success',
    )

    @task(task_id='gcs_6_node_6')
    def run_node_6():
        action = 'delete'
        bucket_name = 'os-dpf-ariflow-prj-dev'
        service_account_file = '/opt/airflow/credgcp.json'
        ...
        object_names = []
        delete_multiple_source_type = 'local'
        delete_object_prefix = 'n8n_vs_langchain_langGraph.docx'
        delete_local_directory_path = 'n8n_vs_langchain_langGraph.docx'
        delete_local_file_paths = []
        delete_drive_input_mode = 'directory'
        delete_drive_directory_link = ''
        delete_drive_file_links = []
    ...
    task_node_4 >> task_node_7
    task_node_7 >> task_node_5
    task_node_7 >> task_node_8
    task_node_5 >> task_node_9
    task_node_8 >> task_node_9
    task_node_9 >> task_node_6
